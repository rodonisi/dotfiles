...
-- Aerospace workspaces - matching your actual configuration
local workspace_names = { "1", "2", "3", "4", "5", "6", "7", "8", "9", "B", "C", "M", "T" }

for i = 1, #workspace_names, 1 do
	...
	space:subscribe("mouse.clicked", function(env)
		if env.BUTTON == "other" then
			...
			if env.BUTTON == "right" then
				...
				sbar.exec("aerospace workspace " .. workspace_name .. " && aerospace close-all-windows-but-current")
			else
					...
					function(result)
						...
						if result == "" or result:match("^%s*$") then
							...
							for line in result:gmatch("[^\r\n]+") do
								local count, app = line:match("(%d+)%s+(.+)")
								if app then
									no_app = false
						...
						sbar.animate("tanh", 5, function()
							...
							local padding_item = sbar.query("space.padding." .. i)
